...
def format_list(seq: Iterator[str]) -> str:
...
def create_parser() -> ArgumentParserMkosi:
    ...
    group.add_argument(
        ...
        help=f"The command line to use for {format_list(verb.name for verb in MKOSI_COMMANDS_CMDLINE)}",
    )
...
def load_args(args: argparse.Namespace) -> MkosiArgs:
    ...
    if args.cmdline and args.verb not in MKOSI_COMMANDS_CMDLINE:
        die(f"Additional parameters only accepted for {format_list(verb.name for verb in MKOSI_COMMANDS_CMDLINE)} invocations.")
