    ...
    with complete_step(f"Writing {suffix} file…"):
        f: BinaryIO = cast(
            ...
            tempfile.NamedTemporaryFile(
                mode="w+b", prefix=".mkosi", dir=os.path.dirname(args.output_root_hash_p7s_file)
            ),
        )
...
def calculate_sha256sum(
    ...
    with complete_step("Calculating SHA256SUMS…"):
        ...
        if root_hash_p7s_file is not None:
            assert args.output_root_hash_p7s_file is not None
            hash_file(f, root_hash_p7s_file, os.path.basename(args.output_root_hash_p7s_file))
        ...
        if split_verity_sig is not None:
            ...
            hash_file(f, split_verity_sig, os.path.basename(args.output_split_verity_sig))
